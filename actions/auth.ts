...
export async function requestPasswordReset(formData: FormData) {
  ...
  try {
    // 1) Comprobar si existe un usuario con ese email. Se hace con el cliente de servidor
    //    porque user_profiles ya no es legible sin sesion.
    const { data: userRecord, error: userError } = await getSupabaseAdminClient()
      .from("user_profiles")
      .select("user_id")
      .eq("email", email)
      .maybeSingle();
